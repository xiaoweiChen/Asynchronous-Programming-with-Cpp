...
回到 C++ 引用，我们看到当协程调用 co\_await 时，编译器将生成代码来调用承诺对象中名为 await\_transform 的函数，该函数具有与协程正在等待的数据相同类型的参数。顾名思义， await\_transform 是一个将任何对象（在我们的示例中为 std::string）转换为可等待对象的函数。 std::string 不可等待，因此出现前面的编译器错误。
